...
if True:
    ...
    f1=open("temp_photos/temp_variable_left.txt", "r")
    directionL =f1.read()

    f2=open("temp_photos/temp_variable_front.txt", "r")
    directionF =f2.read()

    f3=open("temp_photos/temp_variable_right.txt", "r")
    directionR =f3.read()

    #======left==========================================================================
    imcv = cv2.imread('cropped/'+str(directionL)+'_mask_fitted.png', cv2.IMREAD_UNCHANGED)
    unwrapper = LabelUnwrapper(src_image=imcv, percent_points=points)
    dst_image = unwrapper.unwrap()
    cv2.imwrite('cropped/merge/'+str(directionL)+'_mask_fitted.png', dst_image)
    #cv2.imwrite("mask.jpg", unwrapper.get_label_mask())
    #====================================================================================

    #======front==========================================================================
    imcv = cv2.imread('cropped/'+str(directionF)+'_mask_fitted.png', cv2.IMREAD_UNCHANGED)
    unwrapper = LabelUnwrapper(src_image=imcv, percent_points=points)
    dst_image = unwrapper.unwrap()
    cv2.imwrite('cropped/merge/'+str(directionF)+'_mask_fitted.png', dst_image)
    #cv2.imwrite("mask.jpg", unwrapper.get_label_mask())
    #====================================================================================

    #======right==========================================================================
    imcv = cv2.imread('cropped/'+str(directionR)+'_mask_fitted.png', cv2.IMREAD_UNCHANGED)
    unwrapper = LabelUnwrapper(src_image=imcv, percent_points=points)
    dst_image = unwrapper.unwrap()
    cv2.imwrite('cropped/merge/'+str(directionR)+'_mask_fitted.png', dst_image)
    #cv2.imwrite("mask.jpg", unwrapper.get_label_mask())
    #====================================================================================
